...
"""
    ...
    ** Read Python coding style first: http://www.python.org/dev/peps/pep-0008/ **
...
class Writer:
    ...
    def startWriter(self, tag, level, summary, title_msg, special_tag=None, cve_number=""):
        ...
        danger_list = ["Critical", "Warning", "Notice", "Info"]

        if "level" in self.__output_dict_vector_result_information[tag]:
            new_level = level
            old_level = self.__output_dict_vector_result_information[tag]["level"]
            if danger_list.index(new_level) < danger_list.index(old_level):
                self.__output_dict_vector_result_information[tag]["level"] = new_level
                self.__output_dict_vector_result_information[tag]["title"] = title_msg.rstrip('\n')
                self.__output_dict_vector_result_information[tag]["summary"] = summary.rstrip('\n')
        else:
            self.__output_dict_vector_result_information[tag]["level"] = level
            self.__output_dict_vector_result_information[tag]["title"] = title_msg.rstrip('\n')
            self.__output_dict_vector_result_information[tag]["summary"] = summary.rstrip('\n')

        if "count" not in self.__output_dict_vector_result_information[tag].keys():
            self.__output_dict_vector_result_information[tag]["count"] = 0
        if special_tag:
            assert isinstance(special_tag, list), "Tag [" + tag + "]: special_tag should be list"
            self.__output_dict_vector_result_information[tag]["special_tag"] = special_tag    #Notice: the type of "special_tag" is "list"
        if cve_number:
            assert isinstance(cve_number, basestring), "Tag [" + tag + "]: special_tag should be string"
            self.__output_dict_vector_result_information[tag]["cve_number"] = cve_number
    ...
    def completeWriter(self):
        ...
        if self.__cache_output_detail_stream and self.__output_current_tag != "":
            # This is the preferred way if you know that your variable is a string.
            ...
            if current_tag in self.__output_dict_vector_result_information:
                ...
                for line in self.__cache_output_detail_stream:
                    output_string += str(line).encode('string_escape')  # To escape the "\n" shown in the original string inside the APK

                if "vector_details" not in self.__output_dict_vector_result_information[current_tag].keys():
                    self.__output_dict_vector_result_information[current_tag]["vector_details"] = ""
                else:
                    self.__output_dict_vector_result_information[current_tag]["vector_details"] += \
                        self.get_valid_encoding_utf8_string(str('\n').encode('string_escape'))
...
def parseArgument():
    ...
    parser.add_argument("-m", "--analyze_mode", help="Specify \"single\" (default) or \"massive\"",
                        type=str, required=False, default=ANALYZE_MODE_SINGLE)
    ...
    parser.add_argument("-e", "--extra", help="1) Do not check (default)\t"
                                              "2) Check security class names, method names and native methods",
    ...
    parser.add_argument("-s", "--store_analysis_result_in_db",
                        help="Specify this argument if you want to store the analysis result in MongoDB. "
                             "Please add this argument if you have MongoDB connection.", action="store_true")
...
def dex_analysis(writer, args, a, dex, int_min_sdk, int_target_sdk):
    # a = apk
    # ////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if allurls_strip_non_duplicated_final_prerun_count != 0:
        ...
        writer.startWriter("SSL_URLS_NOT_IN_HTTPS", LEVEL_INFO, "SSL Connection Checking",
                           "Did not discover any URLs that are not under SSL (Notice: if the URL string is encrypted, we can not discover that).",
                           ["SSL_Security"])
    ...
    if args.extra == 2:  # The output may be too verbose, so make it an option
        ...
        if list_security_related_classes:
            writer.startWriter("Security_Classes", LEVEL_NOTICE, "Security Classes Checking",
                               "Found some security related class names:")
    ...
    if isMasterKeyVulnerability:
        writer.startWriter("MASTER_KEY", LEVEL_CRITICAL, "Master Key Type I Vulnerability",
                           "This APK contains Master Key Type I Vulnerability.", None, "CVE-2013-4787")
    else:
        writer.startWriter("MASTER_KEY", LEVEL_INFO, "Master Key Type I Vulnerability",
                           "No Master Key Type I Vulnerability found in this APK.", None, "CVE-2013-4787")
    ...
    if is_debug_open:
        writer.startWriter("DEBUGGABLE", LEVEL_CRITICAL, "Android Debug Mode Checking",
                           "DEBUG mode is ON (android:debuggable=\"true\") in AndroidManifest.xml. This is very dangerous. The attackers will be able to sniff the debug messages using Logcat. Please disable the DEBUG mode if it is a released application.",
                           ["Debug"])
    ...
    if list_detected_FLAG_DEBUGGABLE_path:
        ...
        writer.startWriter("HACKER_DEBUGGABLE_CHECK", LEVEL_INFO, "Codes for Checking Android Debug Mode",
                           "Did not detect any codes for checking \"ApplicationInfo.FLAG_DEBUGGABLE\" in AndroidManifest.xml.",
                           ["Debug", "Hacker"])
    ...
    if ACCESS_MOCK_LOCATION in all_permissions:
        ...
        writer.startWriter("USE_PERMISSION_ACCESS_MOCK_LOCATION", LEVEL_INFO, "Unnecessary Permission Checking",
                           "Permission 'android.permission.ACCESS_MOCK_LOCATION' is set correctly.")
    ...
    if pkg_URLConnection or pkg_HttpURLConnection or pkg_HttpsURLConnection or pkg_DefaultHttpClient or pkg_HttpClient:
        ...
        writer.startWriter("USE_PERMISSION_INTERNET", LEVEL_INFO, "Accessing the Internet Checking",
                           "No HTTP-related connection code found.")
    ...
    if organized_list_base64_success_decoded_string_to_original_mapping:  # The result is from the upper code section
        ...
        writer.startWriter("HACKER_BASE64_STRING_DECODE", LEVEL_CRITICAL, "Base64 String Encryption",
                           "Found Base64 encoding \"String(s)\" (Total: " + str(len(
                               organized_list_base64_success_decoded_string_to_original_mapping)) + "). We cannot guarantee all of the Strings are Base64 encoding and also we will not show the decoded binary file:",
                           ["Hacker"])
        ...
        writer.startWriter("HACKER_BASE64_STRING_DECODE", LEVEL_INFO, "Base64 String Encryption",
                           "No Base64 encoded Strings or Urls found.", ["Hacker"])
    ...
    Please modify the below code:"""
        ...
        writer.startWriter("WEBVIEW_RCE", LEVEL_INFO, "WebView RCE Vulnerability Checking",
                           "Did not find any WebView addJavascriptInterface vulnerabilities.",
                           ["WebView", "Remote Code Execution"], "CVE-2013-4710")
    ...
    if (not list_no_pwd_keystore) and (not list_protected_keystore) and (not list_no_pwd_probably_ssl_pinning_keystore):
        ...
        writer.startWriter("HACKER_KEYSTORE_NO_PWD", LEVEL_INFO, "KeyStore Protection Checking",
                           "Ignored checking KeyStore password protection because KeyStore is not used.",
                           ["KeyStore", "Hacker"])
        ...
        if list_no_pwd_probably_ssl_pinning_keystore:
            ...
            writer.startWriter("HACKER_KEYSTORE_SSL_PINNING", LEVEL_CRITICAL, "KeyStore Protection Checking",
                               "The Keystores below seem using \"byte array\" or \"hard-coded cert info\" to do SSL pinning (Total: " + str(
                                   len(list_no_pwd_probably_ssl_pinning_keystore)) + "). Please check manually:",
                               ["KeyStore", "Hacker"])
        ...
        if list_no_pwd_keystore:
            ...
            writer.startWriter("HACKER_KEYSTORE_NO_PWD", LEVEL_CRITICAL, "KeyStore Protection Checking",
                               "The Keystores below seem \"NOT\" protected by password (Total: " + str(
                                   len(list_no_pwd_keystore)) + "). Please check manually:", ["KeyStore", "Hacker"])
    ...
    if list_Non_BKS_keystore:
        writer.startWriter("KEYSTORE_TYPE_CHECK", LEVEL_CRITICAL, "KeyStore Type Checking",
                           "Android only accepts 'BKS' type KeyStore. Please confirm you are using 'BKS' type KeyStore:",
                           ["KeyStore"])
    ...
    if list_PackageInfo_signatures:
        writer.startWriter("HACKER_SIGNATURE_CHECK", LEVEL_NOTICE, "Getting Signature Code Checking",
                           "This app has code checking the package signature in the code. It might be used to check if the app is hacked by the attackers.",
                           ["Signature", "Hacker"])
        ...
        writer.startWriter("HACKER_SIGNATURE_CHECK", LEVEL_INFO, "Getting Signature Code Checking",
                           "Did not detect if this app is checking the signature in the code.", ["Signature", "Hacker"])
    ...
    if list_code_for_preventing_screen_capture:
        writer.startWriter("HACKER_PREVENT_SCREENSHOT_CHECK", LEVEL_NOTICE,
                           ...
                           """This app has code for preventing screenshot capturing. Example: getWindow().setFlags(WindowManager.LayoutParams.FLAG_SECURE, WindowManager.LayoutParams.FLAG_SECURE); It is used by the developers to protect the app:""",
                           ["Hacker"])
        ...
        writer.startWriter("HACKER_PREVENT_SCREENSHOT_CHECK", LEVEL_INFO,
                           "Code Setting Preventing Screenshot Capturing",
                           "Did not detect if this app has code for preventing screenshot capturing.", ["Hacker"])
    ...
    if path_Runtime_exec:
        writer.startWriter("COMMAND", LEVEL_CRITICAL, "Runtime Command Checking",
                           "This app is using critical function 'Runtime.getRuntime().exec(\"...\")'. Please confirm that these following code sections are not harmful:",
                           ["Command"])
        ...
        if list_Runtime_exec:
            writer.startWriter("COMMAND_SU", LEVEL_CRITICAL, "Runtime Critical Command Checking",
                               "Requesting for \"root\" permission code sections 'Runtime.getRuntime().exec(\"su\")' found (Critical but maybe false positives):",
                               ["Command"])
    ...
    if path_getInsecure:
        ...
        writer.startWriter("SSL_CN3", LEVEL_INFO, "SSL Implementation Checking (Insecure component)",
                           "Did not detect SSLSocketFactory using insecure method \"getInsecure\".", ["SSL_Security"])
    ...
    if list_HttpHost_scheme_http:
        ...
        writer.startWriter("SSL_DEFAULT_SCHEME_NAME", LEVEL_INFO, "SSL Implementation Checking (HttpHost)",
                           "DEFAULT_SCHEME_NAME for HttpHost check is OK.", ["SSL_Security"])
    ...
    if list_webviewClient:
        ...
        writer.startWriter("SSL_WEBVIEW", LEVEL_INFO, "SSL Implementation Checking (WebViewClient for WebView)",
                           "Did not detect any critical usage of \"WebViewClient\" (MITM Vulnerability).",
                           ["SSL_Security"])
    ...
    if list_setJavaScriptEnabled_XSS:
        writer.startWriter("WEBVIEW_JS_ENABLED", LEVEL_WARNING, "WebView Potential XSS Attacks Checking",
                           "Found \"setJavaScriptEnabled(true)\" in WebView, which could expose to potential XSS attacks. Please check the web page code carefully and sanitize the output:",
                           ["WebView"])
    ...
    if (int_min_sdk is not None) and (int_min_sdk < 11):
        ...
        if path_SQLiteDatabase_beginTransactionNonExclusive:
            ...
            output_string.append(
                "We detected \"beginTransactionNonExclusive\" being used in \"SQLiteDatabase\" but minSdk supports down to " + str(
                    int_min_sdk) + ". ")
            output_string.append(
            ...
            writer.startWriter("DB_DEPRECATED_USE1", LEVEL_INFO, "SQLiteDatabase Transaction Deprecated Checking",
                               "Ignored checking \"SQLiteDatabase:beginTransactionNonExclusive\" because it's not used.",
                               ["Database"])
    else:
        writer.startWriter("DB_DEPRECATED_USE1", LEVEL_INFO, "SQLiteDatabase Transaction Deprecated Checking",
                           "Ignored checking \"SQLiteDatabase:beginTransactionNonExclusive\" because minSdk >= 11.",
                           ["Database"])
    ...
    if list_NDK_library_classname_to_ndkso_mapping:
        writer.startWriter("NATIVE_LIBS_LOADING", LEVEL_NOTICE, "Native Library Loading Checking",
                           "Native library loading code (System.loadLibrary(...)) found:")
        ...
        writer.startWriter("NATIVE_LIBS_LOADING", LEVEL_INFO, "Native Library Loading Checking",
                           "No native library loading code found.")
    ...
    if list_NDK_library_classname_to_ndkso_mapping:
        ...
        if is_using_Framework_Bangcle:
            writer.startWriter("FRAMEWORK_BANGCLE", LEVEL_NOTICE, "Encryption Framework - Bangcle",
                               "This app is using Bangcle Encryption Framework (http://www.bangcle.com/). Please send your unencrypted apk instead so that we can check it thoroughly.",
                               ["Framework"])
        if is_using_Framework_ijiami:
            writer.startWriter("FRAMEWORK_IJIAMI", LEVEL_NOTICE, "Encryption Framework - Ijiami",
                               "This app is using Ijiami Encryption Framework (http://www.ijiami.cn/). Please send your unencrypted apk instead so that we can check it thoroughly.",
                               ["Framework"])
    ...
    if paths_ExternalStorageAccess:
        writer.startWriter("EXTERNAL_STORAGE", LEVEL_WARNING, "External Storage Accessing",
                           "External storage access found (Remember: DO NOT write important files to external storage):")
    ...
    References:
      ...
      5. http://securityintelligence.com/wp-content/uploads/2013/12/android-collapses-into-fragments.pdf"""
        ...
        writer.startWriter("FRAGMENT_INJECTION", LEVEL_CRITICAL, "Fragment Vulnerability Checking", output_string, None,
                           "BID 64208, CVE-2013-6271")
    ...
    if isUsingSQLCipher:
        writer.startWriter("DB_SQLCIPHER", LEVEL_NOTICE, "Android SQLite Databases Encryption (SQLCipher)",
                           "This app is using SQLCipher (http://sqlcipher.net/) to encrypt or decrypt databases.",
                           ["Database"])
        ...
        writer.startWriter("DB_SQLCIPHER", LEVEL_INFO, "Android SQLite Databases Encryption (SQLCipher)",
                           "This app is \"NOT\" using SQLCipher (http://sqlcipher.net/) to encrypt or decrypt databases.",
                           ["Database"])
    ...
    if has_SSE_databases:
        writer.startWriter("DB_SEE", LEVEL_NOTICE,
                           ...
                           "This app is using SQLite Encryption Extension (SEE) on Android (http://www.sqlite.org/android) to encrypt or decrypt databases.",
                           ["Database"])

    else:
        writer.startWriter("DB_SEE", LEVEL_INFO,
                           "Android SQLite Databases Encryption (SQLite Encryption Extension (SEE))",
                           "This app is \"NOT\" using SQLite Encryption Extension (SEE) on Android (http://www.sqlite.org/android) to encrypt or decrypt databases.",
                           ["Database"])
    ...
    if result_sqlite_encryption:
        writer.startWriter("HACKER_DB_KEY", LEVEL_NOTICE, "Key for Android SQLite Databases Encryption",
                           "Found using the symmetric key (PRAGMA key) to encrypt the SQLite databases. Related code:",
                           ["Database", "Hacker"])
    ...
    if result_possibly_root_total:
        writer.startWriter("COMMAND_MAYBE_SYSTEM", LEVEL_NOTICE, "Executing \"root\" or System Privilege Checking",
                           "The app may have the code checking for \"root\" permission, mounting filesystem operations or monitoring system:",
                           ["Command"])
        ...
        writer.startWriter("COMMAND_MAYBE_SYSTEM", LEVEL_INFO, "Executing \"root\" or System Privilege Checking",
                           "Did not find codes checking \"root\" permission (su) or getting system permission (It's still possible but we did not find out).",
                           ["Command"])
    ...
    if path_Device_id:
        ...
        writer.startWriter("SENSITIVE_DEVICE_ID", LEVEL_WARNING, "Getting IMEI and Device ID",
               ...
               If you want to get an unique id for the device, we suggest you use "Installation" framework in the following article: http://android-developers.blogspot.tw/2011/03/identifying-app-installations.html.""",
        ...
        writer.startWriter("SENSITIVE_DEVICE_ID", LEVEL_INFO, "Getting IMEI and Device ID",
                           "Did not detect if this app is getting the \"device id(IMEI)\" by \"TelephonyManager.getDeviceId()\" approach.",
                           ["Sensitive_Information"])
    ...
    if list_android_id:
        ...
        writer.startWriter("SENSITIVE_SECURE_ANDROID_ID", LEVEL_INFO, "Getting ANDROID_ID",
                           "Did not detect if this app is getting the 64-bit number \"Settings.Secure.ANDROID_ID\".",
                           ["Sensitive_Information"])
...
def __analyze(writer, args):
    ...
    # TODO: return error when no dex file is found inside the apk
    # if len(a.get_dex()) == 0:
    ...
    for dex in a.get_dex():
        dex_analysis(writer, args, a, dex, int_min_sdk, int_target_sdk)
    ###################################################################################################################
